...
pub struct SearchResultWithReplacement {
    ...
    pub replacement: String, // TODO: verify that this is correct when we perform the replacement, if not crash
...
impl FileSearcher {
    ...
    fn convert_regex(search: &SearchType, options: RegexOptions) -> SearchType {
        let mut search_regex_str = match search {
            ...
            SearchType::PatternAdvanced(pattern) => pattern.as_str().to_owned(),
        };
