...
use frep_core::{line_reader, search};
...
struct Args {
    /// Text search with. This will be regex, unless --fixed-strings is used in which case this is a string literal
    #[arg(index = 1)]
    search_text: String,

    /// (Text to replace with. If blank then the search text will be deleted. This can include capture groups if using search regex
    #[arg(index = 2)]
