...
def get_tools [frep_binary: string, search_term: string, replace_term: string] {
    [
        {
            name: "frep",
            command: $"($frep_binary) '($search_term)' '($replace_term)'",
        },
        {
            name: "ripgrep + sd",
...
def compare_results [tool_results: list] {
    let frep_directory = ($tool_results | where name == "frep" | get dir.0)
    mut all_tests_passed = true

    for result in ($tool_results | where name != "frep") {
        let comparison_passed = (compare_directories $frep_directory $result.dir "frep" $result.name)
...
def get_benchmark_repo_path [repo_url: string] {
    let cache_dir = ($nu.home-path | path join ".cache" "frep" "benchmark")
...
def run_benchmark [project_dir: string, search: string, replace: string, frep_binary: string, update_readme: bool, repo_url: string = ""] {
    ...
    let benchmark_tools = get_tools $frep_binary $search $replace
...
def main [mode: string, --update-readme, --repo-url: string = ""] {
    ...
    let frep_binary = ($project_dir | path join "target" "release" "frep")

    if not ($frep_binary | path exists) {
        print $"❌ ERROR: binary not found at ($frep_binary)"
        exit 1
    }

    let search_term = "before"
    let replace_term = "after"
    let all_tools = get_tools $frep_binary $search_term $replace_term
